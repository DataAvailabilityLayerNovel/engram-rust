...
- `tracing`: Enables logging/tracing, which is useful when dealing with nonce
  ...
  format if needed.
- `next`: Enables V4 header support (`HeaderExtension::V4`) alongside legacy
  V3 decoding paths.

## Header Extension Compatibility

The SDK supports both `HeaderExtension::V3` and `HeaderExtension::V4`. If your
network has migrated to V4 headers, you can decode and read `data_root` from
either format without changing client-side block handling logic.

To verify local compatibility after dependency or runtime updates, run:
