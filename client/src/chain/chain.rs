use crate::{
	...
	transaction_options::{MortalityOption, Options},
...
use avail_rust_core::{
	...
	ext::{sp_crypto_hashing, subxt_rpcs::client::RpcParams},
...
impl Chain {
	fn options_with_fresh_mortality(options: Options) -> Options {
		match options.mortality {
			Some(MortalityOption::Full(m)) => options.mortality(MortalityOption::Period(m.period)),
			_ => options,
		}
	}

	fn is_bad_proof_like_error(err: &Error) -> bool {
		let Error::RpcError(rpc_err) = err else {
			return false;
		};
		let msg = rpc_err.to_string().to_lowercase();
		msg.contains("\"code\":1010")
			|| msg.contains("bad signature")
			|| msg.contains("badproof")
			|| msg.contains("invalidtransaction::badproof")
	}

	async fn sign_and_submit_call_once(
		&self,
		signer: &Keypair,
		...
		#[cfg(feature = "tracing")]
		if Self::tx_debug_enabled() {
			let call_bytes = tx_payload.call.as_ref().encode();
			let extra_bytes = tx_payload.extra.encode();
			let additional_bytes = tx_payload.additional.encode();
			let signing_data = tx_payload.signing_data();
			let signing_hash = if signing_data.len() > 256 {
				Some(sp_crypto_hashing::blake2_256(&signing_data))
			} else {
				None
			};
			tracing::info!(
				target: "tx_debug",
				call_hex = %const_hex::encode(&call_bytes),
				extra_hex = %const_hex::encode(&extra_bytes),
				additional_hex = %const_hex::encode(&additional_bytes),
				signing_data_len = signing_data.len(),
				signing_data_hex = %const_hex::encode(&signing_data),
				signing_hash_hex = ?signing_hash.map(const_hex::encode),
				spec_version = tx_additional.spec_version,
				tx_version = tx_additional.tx_version,
				genesis_hash = ?tx_additional.genesis_hash,
				fork_hash = ?tx_additional.fork_hash,
				nonce = tx_payload.extra.nonce,
				app_id = tx_payload.extra.app_id,
				"Prepared signing payload before submit"
			);
		}
	...
	#[cfg(feature = "tracing")]
	fn tx_debug_enabled() -> bool {
		match std::env::var("AVAIL_TX_DEBUG") {
			Ok(v) => {
				let v = v.trim();
	...
	pub async fn build_payload<'a>(
		...
		let refined_options = Self::options_with_fresh_mortality(options)
			.build(&self.client, account_id, self.retry_on_error)
			.await?;
	...
	pub async fn build_extrinsic_from_call<'a>(
		...
		let payload = self
			.build_payload(&account_id, call, Self::options_with_fresh_mortality(options))
			.await?;
		#[cfg(feature = "tracing")]
		if Self::tx_debug_enabled() {
			let call_bytes = payload.call.as_ref().encode();
			let extra_bytes = payload.extra.encode();
			let additional_bytes = payload.additional.encode();
			...
			tracing::info!(
				...
				signing_data_hex = %const_hex::encode(&signing_data),
				signing_hash_hex = ?signing_hash.map(const_hex::encode),
				"Prepared signing payload for extrinsic"
			);
		}
		let signature = payload.sign(signer);

		let tx = avail_rust_core::GenericExtrinsic::new(account_id, signature, payload);
		#[cfg(feature = "tracing")]
		if Self::tx_debug_enabled() {
			let encoded = tx.encode();
			tracing::info!(
				target: "tx_debug",
				extrinsic_len = encoded.len(),
				extrinsic_hex = %const_hex::encode(&encoded),
				"Built signed extrinsic bytes"
			);
		}
		Ok(tx)
	}
	...
	pub async fn submit(&self, ext: &avail_rust_core::GenericExtrinsic<'_>) -> Result<H256, RpcError> {
		...
		let local_tx_hash = H256::from(sp_crypto_hashing::blake2_256(&encoded));
		...
		if let Some(signed) = &ext.signature {
			if let avail_rust_core::MultiAddress::Id(account_id) = &signed.address {
				tracing::info!(
					target: "tx",
					"Submitting Transaction. Address: {}, Nonce: {}, App Id: {}, Local Tx Hash: {:?}",
					account_id,
					signed.extra.nonce,
					signed.extra.app_id,
					local_tx_hash
	...
	pub async fn sign_and_submit_call(
		...
		let options = Self::options_with_fresh_mortality(options);
		let account_id = signer.public_key().to_account_id();
		match self.sign_and_submit_call_once(signer, &account_id, tx_call, options).await {
			Ok(value) => Ok(value),
			Err(err) if Self::is_bad_proof_like_error(&err) => {
				#[cfg(feature = "tracing")]
				tracing::warn!(
					target: "tx",
					"BadProof/1010 detected, retrying once with freshly rebuilt mortality"
				);
				self.sign_and_submit_call_once(
					signer,
					&account_id,
					tx_call,
					Self::options_with_fresh_mortality(options),
				)
				.await
			},
			Err(err) => Err(err),
