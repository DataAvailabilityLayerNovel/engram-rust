...
use crate::{Client, Error, UserError, subxt_core::config::Header};
...
impl RefinedMortality {
	...
	pub async fn from_period(client: &Client, period: u64) -> Result<Self, crate::Error> {
		...
		let birth = match era {
			Era::Immortal => header.number(),
			Era::Mortal { period, phase } => {
				let birth = current.saturating_sub((current.saturating_sub(phase)) % period);
				birth as u32
			},
		};
		let Some(block_hash) = client.chain().block_hash(Some(birth)).await? else {
			return Err(Error::User(UserError::Other(format!(
				"Failed to resolve mortality birth block hash at height {birth}"
			))));
		};
		Ok(Self { period, block_hash, block_height: birth })
	}
}