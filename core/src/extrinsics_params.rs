...
impl<T: Config> transaction_extensions::Params<T> for CheckNonZeroSender {}
...
impl<T: Config> subxt_core::config::ExtrinsicParams<T> for CheckNonZeroSender {
	type Params = ();

	fn new(_client: &ClientState<T>, _params: Self::Params) -> Result<Self, ExtrinsicParamsError> {
		Ok(CheckNonZeroSender)
	}
}

impl ExtrinsicParamsEncoder for CheckNonZeroSender {
	fn encode_value_to(&self, _v: &mut Vec<u8>) {}
	fn encode_implicit_to(&self, _v: &mut Vec<u8>) {}
}

impl<T: Config> transaction_extensions::TransactionExtension<T> for CheckNonZeroSender {
	type Decoded = ();
