...
mod tests {
	...
	#[test]
	fn data_root_reads_from_v4_extension() {
		let expected = H256::repeat_byte(7);
		let header = AvailHeader {
			extension: HeaderExtension::V4(V4HeaderExtension {
				app_lookup: V4CompactDataLookup {
					size: 1,
					index: Vec::new(),
					rows_per_tx: vec![1],
				},
		...
		assert_eq!(header.data_root(), expected);
	}
}
